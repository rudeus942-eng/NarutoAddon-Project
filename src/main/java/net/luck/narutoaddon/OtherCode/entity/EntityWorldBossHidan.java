...
public class EntityWorldBossHidan extends ElementsInfTsukAddon.ModElement {
   public static final int ENTITYID = 305;

   public EntityWorldBossHidan(ElementsInfTsukAddon instance) {
      super(instance, 961);
   }

   public void initElements() {
      this.elements.entities.add((Supplier)() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("inftsukaddon", "worldbosshidan"), 305).name("worldbosshidan").tracker(64, 1, true).egg(-1, -1).build());
   }

   @SideOnly(Side.CLIENT)
   public void preInit(FMLPreInitializationEvent event) {
      RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, WorldBossHidanRenderer::new);
   }

   @SideOnly(Side.CLIENT)
   public static class WorldBossHidanRenderer extends RenderLiving<EntityCustom> {
      private static final ResourceLocation FALLBACK_TEXTURE = new ResourceLocation("inftsukaddon:textures/hidan.png");

      public WorldBossHidanRenderer(RenderManager renderManager) {
   ...
   public static class EntityCustom extends QuestNpcBase {
      private static final float WB_CD_MUL = 0.6F;
      private int phase = 1;
      ...
      protected void processCombat(EntityLivingBase target, double dist) {
         if (target != null && target.isEntityAlive()) {
            ...
            if (!this.introPlayed) {
               ...
               this.hidanSay("§4§lHeh... A worthy sacrifice for Lord Jashin! Prepare to die!");
            }
            ...
            if (this.comboStep > 0 && this.comboTarget != null) {
               ...
               float cdMul = this.phaseCdMul() * 0.6F;
               ...
               if (this.hasResurrected && this.jashinCurseCD <= 0 && dist <= (double)6.0F && !this.world.isRemote) {
                  this.jashinCurseCD = (int)(120.0F * this.phaseCdMul() * 0.6F);
               ...
               if (this.hasResurrected && this.selfHarmAoeCD <= 0 && dist <= (double)10.0F && !this.world.isRemote && this.rand.nextFloat() < 0.35F) {
                  this.selfHarmAoeCD = (int)(100.0F * this.phaseCdMul() * 0.6F);
      ...
      private float phaseCdMul() {
         float base = this.phase >= 3 ? 0.5F : (this.phase >= 2 ? 0.7F : 1.0F);
      ...
      private void performScytheMelee(EntityLivingBase target, float baseDmg, double dmgMul) {
         ...
         float cdMul = this.phaseCdMul() * 0.6F;
         int[] cd = this.getCooldownRange((int)(20.0F * cdMul), (int)(30.0F * cdMul));
         this.meleeCD = cd[0] + this.rand.nextInt(cd[1] - cd[0] + 1);
      }
      ...
      protected void writeCombatNBT(NBTTagCompound compound) {
         compound.setInteger("wbHidanPhase", this.phase);
         compound.setInteger("wbHidanAttackTick", this.attackTick);
         compound.setBoolean("wbHidanIntroPlayed", this.introPlayed);
         compound.setInteger("wbHidanMeleeCD", this.meleeCD);
         compound.setInteger("wbHidanScytheThrowCD", this.scytheThrowCD);
         compound.setInteger("wbHidanRitualCircleCD", this.ritualCircleCD);
         compound.setInteger("wbHidanAoeSlashCD", this.aoeSlashCD);
         compound.setInteger("wbHidanAntiYCheeseCD", this.antiYCheeseCD);
         compound.setInteger("wbHidanJashinCurseCD", this.jashinCurseCD);
         compound.setInteger("wbHidanSelfHarmAoeCD", this.selfHarmAoeCD);
         compound.setBoolean("wbHidanHasResurrected", this.hasResurrected);
         compound.setInteger("wbHidanResInvuln", this.resurrectionInvulnTicks);
         compound.setBoolean("wbHidanPhase2Announced", this.phase2Announced);
         compound.setBoolean("wbHidanPhase3Announced", this.phase3Announced);
      }

      protected void readCombatNBT(NBTTagCompound compound) {
         this.phase = compound.hasKey("wbHidanPhase") ? compound.getInteger("wbHidanPhase") : 1;
         this.attackTick = compound.hasKey("wbHidanAttackTick") ? compound.getInteger("wbHidanAttackTick") : 0;
         this.introPlayed = compound.getBoolean("wbHidanIntroPlayed");
         this.meleeCD = compound.hasKey("wbHidanMeleeCD") ? compound.getInteger("wbHidanMeleeCD") : 0;
         this.scytheThrowCD = compound.hasKey("wbHidanScytheThrowCD") ? compound.getInteger("wbHidanScytheThrowCD") : 0;
         this.ritualCircleCD = compound.hasKey("wbHidanRitualCircleCD") ? compound.getInteger("wbHidanRitualCircleCD") : 0;
         this.aoeSlashCD = compound.hasKey("wbHidanAoeSlashCD") ? compound.getInteger("wbHidanAoeSlashCD") : 0;
         this.antiYCheeseCD = compound.hasKey("wbHidanAntiYCheeseCD") ? compound.getInteger("wbHidanAntiYCheeseCD") : 0;
         this.jashinCurseCD = compound.hasKey("wbHidanJashinCurseCD") ? compound.getInteger("wbHidanJashinCurseCD") : 0;
         this.selfHarmAoeCD = compound.hasKey("wbHidanSelfHarmAoeCD") ? compound.getInteger("wbHidanSelfHarmAoeCD") : 0;
         this.hasResurrected = compound.getBoolean("wbHidanHasResurrected");
         this.resurrectionInvulnTicks = compound.hasKey("wbHidanResInvuln") ? compound.getInteger("wbHidanResInvuln") : 0;
         this.phase2Announced = compound.getBoolean("wbHidanPhase2Announced");
         this.phase3Announced = compound.getBoolean("wbHidanPhase3Announced");
      }
