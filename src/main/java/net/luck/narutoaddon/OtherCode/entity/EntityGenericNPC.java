...
public class EntityGenericNPC extends ElementsInfTsukAddon.ModElement {
   public static final int ENTITYID = 19;

   public EntityGenericNPC(ElementsInfTsukAddon instance) {
      super(instance, 37);
   }

   public void initElements() {
      this.elements.entities.add((Supplier)() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("inftsukaddon", "questnpc1"), 19).name("questnpc1").tracker(64, 3, true).egg(-1, -1).build());
   }

   @SideOnly(Side.CLIENT)
   public void preInit(FMLPreInitializationEvent event) {
      RenderingRegistry.registerEntityRenderingHandler(EntityCustom.class, GenericNpcRenderer::new);
   }

   @SideOnly(Side.CLIENT)
   public static class GenericNpcRenderer extends RenderLiving<EntityCustom> {
      private static final ResourceLocation FALLBACK_TEXTURE = new ResourceLocation("inftsukaddon:textures/bandit1.png");

      public GenericNpcRenderer(RenderManager renderManager) {
   ...
   public static class EntityCustom extends QuestNpcBase {
      private static final int HEADHUNTER_COOLDOWN_BASE = 400;
      ...
      private int feralState = 0;
      private int feralStateTicks = 0;
      private int feralDecisionCooldown = 0;
      private boolean feralJukeLeft = true;
      private static final int FERAL_CHARGE = 1;
      private static final int FERAL_FLEE = 2;
      private static final int FERAL_JUKE = 3;
      private static final int FERAL_STARE = 4;
      ...
      protected void processCombat(EntityLivingBase target, double dist) {
         if (target != null && target.isEntityAlive()) {
            ...
            } else if ("FERAL".equals(this.combatStyle)) {
               this.checkRageTrigger();
               this.processFeralBehavior(target, dist);
            } else {
      ...
      protected void tickStyleCooldowns() {
         if (this.feralDecisionCooldown > 0) {
            --this.feralDecisionCooldown;
         }
      ...
      protected void resetCombatState() {
         ...
         this.feralState = 0;
         this.feralStateTicks = 0;
         this.feralDecisionCooldown = 0;
      }
      ...
      protected void writeCombatNBT(NBTTagCompound compound) {
         ...
         compound.setInteger("feralState", this.feralState);
      }
      ...
      protected void readCombatNBT(NBTTagCompound compound) {
         ...
         this.feralState = compound.hasKey("feralState") ? compound.getInteger("feralState") : 0;
      }
      ...
      private void processHeadhunter() {
         ...
         if (this.headhunterTarget != null && this.headhunterTarget.isEntityAlive()) {
            if (this.headhunterTick <= 15) {
               ...
               this.headhunterTarget.attackEntityFrom(DamageSource.MAGIC, trueDmg);
               this.headhunterTarget.motionX = (double)0.0F;
         ...
         if (this.hiddenMistTarget != null && this.hiddenMistTarget.isEntityAlive()) {
            if (this.hiddenMistTick <= 60) {
               ...
               if (this.hiddenMistTick == 61) {
                  ...
                  float baseDamage = (float)this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue();
                  float ambushDmg = baseDamage * 1.5F;
                  this.hiddenMistTarget.attackEntityFrom(DamageSource.causeMobDamage(this), ambushDmg);
      ...
      private void processFeralBehavior(EntityLivingBase target, double dist) {
         if (this.feralStateTicks > 0) {
            --this.feralStateTicks;
            this.executeFeralState(target, dist);
         } else if (this.feralDecisionCooldown > 0) {
            this.getNavigator().tryMoveToEntityLiving(target, (double)1.0F);
         } else {
            float roll = this.rand.nextFloat();
            float chargeWeight = this.rageTriggered ? 0.35F : 0.3F;
            float fleeWeight = this.rageTriggered ? 0.1F : 0.2F;
            float jukeWeight = 0.25F;
            if (roll < chargeWeight) {
               this.feralState = 1;
               this.feralStateTicks = 15 + this.rand.nextInt(6);
            } else if (roll < chargeWeight + fleeWeight) {
               this.feralState = 2;
               this.feralStateTicks = 20 + this.rand.nextInt(11);
            } else if (roll < chargeWeight + fleeWeight + jukeWeight) {
               this.feralState = 3;
               this.feralStateTicks = 12 + this.rand.nextInt(7);
               this.feralJukeLeft = this.rand.nextBoolean();
            } else {
               this.feralState = 4;
               this.feralStateTicks = 25 + this.rand.nextInt(11);
            }
      ...
      private void executeFeralState(EntityLivingBase target, double dist) {
         switch (this.feralState) {
            case 1:
               this.getNavigator().tryMoveToEntityLiving(target, (double)2.0F);
               if (this.world instanceof WorldServer && this.ticksExisted % 2 == 0) {
                  ((WorldServer)this.world).spawnParticle(EnumParticleTypes.REDSTONE, this.posX, this.posY + 0.2, this.posZ, 3, 0.2, 0.1, 0.2, (double)0.0F, new int[0]);
               }

               if (dist <= (double)2.5F && this.meleeCooldown <= 0) {
                  this.performMeleeSwing(target);
                  this.feralStateTicks = 0;
                  this.feralDecisionCooldown = 15 + this.rand.nextInt(16);
               }
               break;
            case 2:
               double awayX = this.posX + (this.posX - target.posX) * 0.4;
               double awayZ = this.posZ + (this.posZ - target.posZ) * 0.4;
               this.getNavigator().tryMoveToXYZ(awayX, this.posY, awayZ, 1.6);
               break;
            case 3:
               double dx = target.posX - this.posX;
               double dz = target.posZ - this.posZ;
               ...
               if (len > (double)0.5F) {
                  double perpX = this.feralJukeLeft ? -dz / len : dz / len;
                  double perpZ = this.feralJukeLeft ? dx / len : -dx / len;
                  this.motionX = perpX * 0.3;
                  this.motionZ = perpZ * 0.3;
                  this.velocityChanged = true;
               }

               if (dist <= (double)2.5F && this.meleeCooldown <= 0 && this.feralStateTicks <= 3) {
                  this.performMeleeSwing(target);
                  this.feralDecisionCooldown = 10 + this.rand.nextInt(11);
               }
               break;
            case 4:
               this.getLookHelper().setLookPositionWithEntity(target, 30.0F, 30.0F);
               this.getNavigator().clearPath();
               if (this.feralStateTicks <= 5 && dist <= (double)2.5F && this.meleeCooldown <= 0) {
                  this.performMeleeSwing(target);
                  this.feralDecisionCooldown = 20 + this.rand.nextInt(11);
               }
         }
         ...
         if (allies.isEmpty()) {
            ...
            if (totalAttackers < 2) {
               ...
               for(int i = 0; i < allies.size(); ++i) {
                  EntityCustom ally = (EntityCustom)allies.get(i);
                  allyAngles[i] = Math.atan2(ally.posZ - target.posZ, ally.posX - target.posX);
               }
