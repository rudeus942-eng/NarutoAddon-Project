...
public class FMLNetworkHandler {
    ...
    public static String checkModList(Map<String, String> listData, Side side) {
        List<Pair<ModContainer, String>> rejects = NetworkRegistry.INSTANCE.registry().entrySet().stream().map(entry -> Pair.of(entry.getKey(), entry.getValue().checkCompatible(listData, side))).filter(pair -> pair.getValue() != null).sorted(Comparator.comparing(o -> o.getKey().getName())).collect(Collectors.toList());
        if (rejects.isEmpty()) {
            return null;
        } else {
    ...
    private static void ensureRegistryChannels() {
        try {
            Field field = NetworkRegistry.class.getDeclaredField("channels");
            field.setAccessible(true);
            Object value = field.get(NetworkRegistry.INSTANCE);
            EnumMap<Side, Map<String, FMLEmbeddedChannel>> map;
            if (value instanceof EnumMap) {
                map = (EnumMap<Side, Map<String, FMLEmbeddedChannel>>) value;
            } else {
                map = Maps.newEnumMap(Side.class);
            }
            if (map.get(Side.CLIENT) == null) {
                map.put(Side.CLIENT, Maps.newConcurrentMap());
            }
            if (map.get(Side.SERVER) == null) {
                map.put(Side.SERVER, Maps.newConcurrentMap());
            }
            field.set(NetworkRegistry.INSTANCE, map);
        } catch (Exception ignored) {
        }
    }

    public static void registerChannel(FMLContainer container, Side side) {
        ensureRegistryChannels();
        EnumMap<Side, FMLEmbeddedChannel> pair = Maps.newEnumMap(Side.class);
        try {
            Field field = NetworkRegistry.class.getDeclaredField("channels");
            field.setAccessible(true);
            EnumMap<Side, Map<String, FMLEmbeddedChannel>> registry = (EnumMap<Side, Map<String, FMLEmbeddedChannel>>) field.get(NetworkRegistry.INSTANCE);
            for (Side value : Side.values()) {
                Map<String, FMLEmbeddedChannel> map = registry.get(value);
                if (map == null) {
                    map = Maps.newConcurrentMap();
                    registry.put(value, map);
                }
                FMLEmbeddedChannel channel = new FMLEmbeddedChannel(container, "FML", value, new FMLRuntimeCodec(), new HandshakeCompletionHandler());
                map.put("FML", channel);
                pair.put(value, channel);
            }
            field.set(NetworkRegistry.INSTANCE, registry);
            System.out.println("NetworkRegistry channels=" + registry);
        } catch (Exception ignored) {
        }
        channelPair = pair;
        EmbeddedChannel embeddedChannel = channelPair.get(Side.SERVER);
